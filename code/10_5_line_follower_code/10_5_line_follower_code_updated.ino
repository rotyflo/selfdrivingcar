...
const int S_pin = 0; //proportional control
const int P_pin = 1; //proportional control
const int I_pin = 2; //integral control
const int D_pin = 3; //derivative control
...
void CalcError() {
  ...
  for (int ii = 0; ii < 7; ii = ii + 1) { // loop over all of the LDRs
    if (MxRead < LDR[ii]) { // if LDR value is greater than current max
      ...
      MxIndex = -1 * (ii-3);
      im1 = (float)ii;   // IN THE ORIGINAL FILE, SHOWS ii, NOT i. Problem?
    }
